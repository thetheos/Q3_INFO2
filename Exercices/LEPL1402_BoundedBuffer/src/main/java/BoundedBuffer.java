...
public class BoundedBuffer {
    ...
    private void doPut(Integer x){
        ...
        this.size++;
        if(this.putPointer>=this.data.length) this.putPointer = 0;
        this.notify();
    }
    ...
    private Integer doTake(){
        this.size--;
        int dataToTransmit = this.data[this.takePointer++];
        this.notify();
        if (this.takePointer >= this.data.length) this.takePointer = 0;
        this.notify();
